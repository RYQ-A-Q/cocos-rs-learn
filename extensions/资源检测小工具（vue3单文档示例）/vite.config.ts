...
const bundles = {
  main:  'main.ts',
  assetMenu: 'assetMenu.ts',
  defaultPanel: 'panels/default/index.ts',
  heapPanel: 'panels/heap/index.ts',
  bundlePanel: 'panels/bundle/index.ts',
  fguiPanel: 'panels/fgui/index.ts',
...
function ModifyPluginVueExportHelper() {
  return {
          ...
          if(matches) {
            ...
            while(match = matches.next().value) {
              ...
              if(realName) {
                ...
                // console.log('realName', file.name, upLevel, realName)
                if(upLevel > 0) {
    ...
    //       writeFileSync(newPath, fs.readFileSync(oldPath))
    //       // 删除旧文件
    //       unlinkSync(oldPath)
    //     }
    //   })      
  ...
  return {
    ...
    closeBundle() {
      if(css) {
        ...
        fs.writeFileSync(resolve(__dirname, './dist/assets/index.css'), css)
      }
    }
  }
}  
...
function MoveVueExportHelper() {
  return {
    ...
    closeBundle() {
      ...
      let commonDir = resolve(__dirname, './dist/common')
    ...
    generateBundle(options, bundle) {
      for (const fileName in bundle) {
        ...
        if (file.type != 'asset' && file.code) {
          file.code = file.code.replace(/['"]@__ASSET__(.*?)['"]/gi, `__PLUGIN_PATH__ + \"/assets$1\"`)
          file.code = file.code.replace(/['"]@__ROOT__(.*?)['"]/gi, `__PLUGIN_PATH__ + \"$1\"`)
        }
      }
    }
  }
